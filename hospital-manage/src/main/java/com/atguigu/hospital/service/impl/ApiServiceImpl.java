...
public class ApiServiceImpl implements ApiService {
    ...
    public JSONObject getHospital() {
        ...
        if(null != respone && 200 == respone.getIntValue("code")) { //注意获取code属性用的是getIntValue()，说明code类型是数值类型的
            JSONObject jsonObject = respone.getJSONObject("data");//而获取data属性时用的时getJSONObject，说明data属性不是数值也不是字符串的，而是json对象类型的
            return jsonObject;
        }
        return null;
    }
    ...
    public boolean saveHospital(String data) {
        //转换成JSONObject类型之后，它有一个好处：就是你可以从这种对象类型中根据 Key 来取值，
        JSONObject jsonObject = JSONObject.parseObject(data);

        Map<String, Object> paramMap = new HashMap<>();
        paramMap.put("hoscode","10000");
        paramMap.put("hosname",jsonObject.getString("hosname")); //比如根据 hosname 取 hosname对应的值
